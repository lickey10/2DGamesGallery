...
/// </summary>
public class GameManager_FreeShot : Singleton_FreeShot<GameManager_FreeShot> 
{
	[SerializeField] public List<AAColor_FreeShot> aaColors;

	public delegate void ColorChange (AAColor_FreeShot c);
	/// <summary>
	/// Event triggered when we change the colors of the game
	/// </summary>
	public static event ColorChange OnColorChanged;

	private AAColor_FreeShot _currentAAColor;
	public AAColor_FreeShot currentAAColor
	{
		get
		{
	...
	{
		if (PlayerPrefs.GetInt (Constant_FreeShot.SOUND_ON, 1) == 0)
			return;
    ...
    /// </summary>
    public List<DotManager_FreeShot> Dots;
	...
	DotManager_FreeShot[] movingDots; 
	...
	/// list of all the dots the player have to shoot in the level
	/// </summary>
	public List<DotManager_FreeShot> lastShoots;
	...
	void Awake()
	{
		...
		InputTouch_FreeShot.OnTouchScreen -= OnTouch;
		InputTouch_FreeShot.OnTouchScreen += OnTouch;
	}
	...
	public void GameOver()
	{
		...
		CanvasManager_FreeShot.Instance.AnimationCameraGameOver ();
	}
	...
	{
		foreach(var d in FindObjectsOfType<DotManager_FreeShot>())
		{
		...
		Level = PlayerPrefs.GetInt (Constant_FreeShot.LAST_LEVEL_PLAYED, 1);
		this.LEVEL = LevelManager_FreeShot.Instance.GetLevel (Level);
	...
	void PreparePoolForAnimation()
	{
		...
		var dotPool = FindObjectsOfType<DotManager_FreeShot>();
	...
	public void OnDespawnedDotOnTarget(Transform d)
	{
        ...
        if (ExplosionPrefab)
        {
            ...
            t.position = new Vector3(d.position.x, d.position.y, 0);
		...
		if (success && !isGameOver) 
		{
			PlaySoundSuccess();
			isGameOver = true;
			CanvasManager_FreeShot.Instance.AnimationCameraSuccess();
		}
	}
	/// <summary>
	...
	void SmoothDecal()
	{
		...
		{
			movingDots = container.GetComponentsInChildren<DotManager_FreeShot>();

			foreach(DotManager_FreeShot d in movingDots)
			{
