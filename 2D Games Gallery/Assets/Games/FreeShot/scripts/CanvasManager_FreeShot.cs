...
/// </summary>
public class CanvasManager_FreeShot : Singleton_FreeShot<CanvasManager_FreeShot> 
{
	...
	int maxLevel
	{
		get 
		{
			return PlayerPrefs.GetInt (Constant_FreeShot.LEVEL_UNLOCKED, 1);
		}
	}
	...
	int lastLevel
	{
		get 
		{
			return PlayerPrefs.GetInt (Constant_FreeShot.LAST_LEVEL_PLAYED, 1);
		}
	}
	...
	void SetButtons()
	{
		...
		buttonUnlock.onClick.AddListener (() => {
			...
			GameManager_FreeShot.Instance.RefreshColors();
		});
		...
		int soundOn = PlayerPrefs.GetInt(Constant_FreeShot.SOUND_ON,1);
	...
	void TurnSound()
	{
		...
		{
			music.Stop ();
			PlayerPrefs.SetInt (Constant_FreeShot.SOUND_ON, 0);
			buttonSound.transform.GetChild (0).gameObject.SetActive (false);
			buttonSound.transform.GetChild (1).gameObject.SetActive (true);
		}
		else 
		{
			music.Play ();
			PlayerPrefs.SetInt (Constant_FreeShot.SOUND_ON, 1);
			buttonSound.transform.GetChild (0).gameObject.SetActive (true);
			buttonSound.transform.GetChild (1).gameObject.SetActive (false);
		}
	...
	void Awake()
	{
		...
		if (!PlayerPrefs.HasKey (Constant_FreeShot.LAST_LEVEL_PLAYED)) {
			PlayerPrefs.SetInt (Constant_FreeShot.LAST_LEVEL_PLAYED, 1);
		} 

		if (!PlayerPrefs.HasKey (Constant_FreeShot.LEVEL_UNLOCKED)) {
			PlayerPrefs.SetInt (Constant_FreeShot.LEVEL_UNLOCKED, 1);
		} 
	...
	private void ShowRewardedVideoGameOver(){
		...
		GameManager_FreeShot.Instance.success = false;
		GameManager_FreeShot.Instance.isGameOver = false;
	...
	void SetButtonActive(Button b,bool isActive)
	{
		if (isActive) 
		{
			if(GameManager_FreeShot.Instance != null && GameManager_FreeShot.Instance.currentAAColor != null)
			{
				b.GetComponent<Image> ().color = GameManager_FreeShot.Instance.currentAAColor.colorBright;
			}
			b.interactable = true;
		} 
		else
	...
	{
		var f = FindObjectsOfType<DotManager_FreeShot>();

		foreach(var d in f)
		{
			d.DotSprite.DOFade(a,Constant_FreeShot.DELAY_TRANSITION);
		}
	}
	...
	void DOFadeDotManagerWithDelay(float a)
	{
		...
		foreach(var d in f)
		{
			d.DotSprite.DOFade(a,Constant_FreeShot.DELAY_TRANSITION).SetDelay(Constant_FreeShot.DELAY_TRANSITION);
		}
	}
	...
	public void AnimationCameraGameOver()
	{
		...
		GameManager_FreeShot.Instance.canShoot = false;

		ShowButtonUnlock();



		Camera.main.transform.DOShakePosition (Constant_FreeShot.DELAY_TRANSITION, 2, 20, 90, false)
			.OnComplete(() => {
	...
	private void PlayLevel(int level, bool fromSuccess)
	{
		...
		if(level > maxLevel)
			PlayerPrefs.SetInt (Constant_FreeShot.LEVEL_UNLOCKED, level);

		PlayerPrefs.SetInt (Constant_FreeShot.LAST_LEVEL_PLAYED, level);

		PlayerPrefs.Save ();

		ButtonLogic ();

		GameManager_FreeShot.Instance.CreateGame (level, fromSuccess);
