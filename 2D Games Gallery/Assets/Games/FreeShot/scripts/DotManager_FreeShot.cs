...
/// </summary>
public class DotManager_FreeShot : MonoBehaviour 
{
	...
	void Reset()
	{
		...
		DotSprite.color = GameManager_FreeShot.Instance.currentAAColor.colorDark;
	...
	void OnCollisionEnter2D(Collision2D col)
	{
		...
		{
			GameManager_FreeShot.Instance.OnDespawnedDotOnTarget (transform);
			GameManager_FreeShot.Instance.lastShoots.Remove(this);
		}
		else
		{
	...
	{
		if( !GameManager_FreeShot.Instance.isGameOver && !isOnCircle)
		{
			GetComponent<Rigidbody2D>().velocity = Vector3.zero;

			obj.GetComponent<Rigidbody2D>().velocity = Vector3.zero;

			GameManager_FreeShot.Instance.GameOver ();
		}
	}
}
