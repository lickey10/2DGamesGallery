...
public class PlayerController_SpringAnimals : MonoBehaviour {

    public static PlayerController_SpringAnimals instance;
    ...
    {   //depending on selected player we change its sprite by refering to Shop script
        spriteHolder.GetComponent<SpriteRenderer>().sprite = ShopManager_SpringAnimals.instance.shopItems[GameManager_SpringAnimals.instance.selectedSkin].characterSprite;
    }
    ...
    void OnTriggerEnter2D(Collider2D other)                                                         //method called when any object enter the player collider
    {
        if (other.CompareTag("Platform"))                                                           //check if the tag is "Platform"
        {
            ...
            if (script.Scored == false)                                                             //check if the scored is false
            {
                ...
                GameManager_SpringAnimals.instance.currentScore += (script.PlatformIndex - GameManager_SpringAnimals.instance.currentScore); //increase the score
                UIManager.instance.inGameScore.text = "" + GameManager_SpringAnimals.instance.currentScore;       //update the score text
        ...
        if (other.CompareTag("Coin"))                                                               //check if the tag is "Coin"
        {
            ...
            GameManager_SpringAnimals.instance.currentCoins++;                                                    //increase the current coin value
            GameManager_SpringAnimals.instance.coins++;                                                           //increase the total coins value
            UIManager.instance.inGameCoinText.text = "" + GameManager_SpringAnimals.instance.currentCoins;        //set the coin text value
            GameManager_SpringAnimals.instance.Save();                                                            //save the data
        }
        ...
        if (other.CompareTag("Enemy"))                                                              //check if the tag is "Enemy"
        {
            ...
            GameManager_SpringAnimals.instance.isGameOver = true;                                                 //set gameover to true
