...
public class UnityAds_SpringAnimals : MonoBehaviour
{
    public static UnityAds_SpringAnimals instance;
    ...
    void Update()
    {
        if (GameManager_SpringAnimals.instance.isGameOver == true && GameManager_SpringAnimals.instance.canShowAds == true)
        {
            //we want only one ad to be shown so we put condition that when i is 0 we show ad.
            if (GameManager_SpringAnimals.instance.gamesPlayed >= vars.showInterstitialAfter)
            {
                GameManager_SpringAnimals.instance.gamesPlayed = 0;
