...
public class LevelController : MonoBehaviour {
	...
	void Start ()
    {
        GameManager_SpringAnimals.instance.isGameOver = false;            //we set gameover false at start of game
    ...
    void PositionPlatform(float posX)
    {
        ...
        {   
            GameObject platform = ObjectPooling_SpringAnimals.instance.GetPlatform();                    //if yes we get the platform obj from objectpooling
	...
	void Update ()
    {
        if (!gameStarted || GameManager_SpringAnimals.instance.isGameOver)                               //we check if game is over of game is not started , then return
            return;
        ...
        PlayerController_SpringAnimals.instance.Compress(chargePower);                                   //we set compression value for player depending on chargepower value
        ...
        {
            PlayerController_SpringAnimals.instance.Jump(maxJumpForce * chargePower + 50);              //we make player jump
