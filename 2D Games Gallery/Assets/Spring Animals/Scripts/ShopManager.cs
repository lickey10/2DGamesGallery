...
public class ShopManager_SpringAnimals : MonoBehaviour {

    public static ShopManager_SpringAnimals instance;
	...
	void Start ()
    {
        coinText.text = "" + GameManager_SpringAnimals.instance.coins;                        //update the coin text
    }

    public void OpenShop()                                                      //called when shop is opened
    {
        UIManager.instance.ClickSound();
        for (int i = 0; i < GameManager_SpringAnimals.instance.skinUnlocked.Length; i++)      //loop throungh all the shop item
        {
            shopItems[i].unLocked = GameManager_SpringAnimals.instance.skinUnlocked[i];       //set there unlock value with respective to the store value in GameManager data
        }
        currentCharcter = GameManager_SpringAnimals.instance.selectedSkin;                    //ge the selected character and store that value to currentCharacter
    ...
    void BtnCode()
    {
        ...
        if (shopItems[currentCharcter].unLocked && currentCharcter != GameManager_SpringAnimals.instance.selectedSkin)      //if character is unlocked and its not selected
        {
            lockObj.SetActive(false);                                                                         //deactivate lock gameobject
            selectBtnText.text = "Select";                                                                    //set the btn text to select
        }
        
        else if (shopItems[currentCharcter].unLocked && currentCharcter == GameManager_SpringAnimals.instance.selectedSkin) //if character is unlocked and its  selected
        {
            lockObj.SetActive(false);                                                                         //deactivate lock gameobject
    ...
    public void SelectBtn()                                                                                   //called when select btn is clicked
    {
        ...
        {
            if (GameManager_SpringAnimals.instance.coins >= shopItems[currentCharcter].characterCost)                       //we check if we have enought coins to buy
            {   
                GameManager_SpringAnimals.instance.coins -= shopItems[currentCharcter].characterCost;                       //we then reduce the total coins by the cost of character
                shopItems[currentCharcter].unLocked = true;                                                   //unlcok it in shopItems
                GameManager_SpringAnimals.instance.skinUnlocked[currentCharcter] = true;                                    //unlcok it in GameManager
                GameManager_SpringAnimals.instance.selectedSkin = currentCharcter;                                          //set it as selected character
                GameManager_SpringAnimals.instance.Save();                                                                  //Save the data on device
                
                lockObj.SetActive(false);                                                                     //deactivate lock object
                selectBtnText.text = "Selected";                                                              //set select btn text to Selected
                coinText.text = "" + GameManager_SpringAnimals.instance.coins;                                              //update conis text
                PlayerController_SpringAnimals.instance.ChangeSprite();                                                     //and change the player sprite
            }
            else if (GameManager_SpringAnimals.instance.coins < shopItems[currentCharcter].characterCost)                   //if we dont have enough money
        ...
        {   
            if (currentCharcter == GameManager_SpringAnimals.instance.selectedSkin)                                         //if the current charater is equal to selected skin
                return;                                                                                       //do nothing
            
            else if (currentCharcter != GameManager_SpringAnimals.instance.selectedSkin)                                    //if the current charater is not equal to selected skin
            {   
                GameManager_SpringAnimals.instance.selectedSkin = currentCharcter;                                          //select the character
                GameManager_SpringAnimals.instance.Save();                                                                  //save it
                selectBtnText.text = "Selected";                                                              //change the text
                PlayerController_SpringAnimals.instance.ChangeSprite();
            }
        }
    }
