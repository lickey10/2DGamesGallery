...
public class ShopManager : MonoBehaviour {
	...
	void Update ()
    {
        ...
        {   //if yes then we set the position of character images 
            ...
            //set the button 
            if (GameManager_AlienHop.instance.skinUnlocked[characterIndex] == true)
            {
                shopPlay.SetActive(true);
                shopBuy.SetActive(false);
            }
            else if (GameManager_AlienHop.instance.skinUnlocked[characterIndex] == false)
            {
    ...
    public void SelectCharacter()
    {
        ...
        if (GameManager_AlienHop.instance.skinUnlocked[characterIndex] == true)
        {
            GameManager_AlienHop.instance.selectedSkin = characterIndex;
            GameManager_AlienHop.instance.Save();
            string sceneName = SceneManager.GetActiveScene().name;
            SceneManager.LoadScene(sceneName);
        }
        else if (GameManager_AlienHop.instance.points >= vars.characters[characterIndex].characterPrice)
        {
            GameManager_AlienHop.instance.points -= vars.characters[characterIndex].characterPrice;
            GameManager_AlienHop.instance.skinUnlocked[characterIndex] = true;
            GameManager_AlienHop.instance.selectedSkin = characterIndex;
            GameManager_AlienHop.instance.Save();
    ...
    {   //set the scrollContent parent size
        ...
        {   //spawn the prefab
            ...
            {   //set the image value
                ...
                if (GameManager_AlienHop.instance.skinUnlocked[i] == false)
        ...
        shopScroll.content.anchoredPosition = new Vector2(-(GameManager_AlienHop.instance.selectedSkin * scrollItemWidth), 0f);
    }
