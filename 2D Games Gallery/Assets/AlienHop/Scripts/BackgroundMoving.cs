...
public class BackgroundMoving : MonoBehaviour {
    ...
    void Update ()
    {
        if (GameManager_AlienHop.instance.gameOver)//when game is over we save the time
            GameManager_AlienHop.instance.timeDiff = Time.time;
        //if game is over or player is not moving and move with player is true
        if (GameManager_AlienHop.instance.gameOver || PlayerControl.instance.StartMovingBool == false
            && moveWithPlayer == true)//we return
            return;
        //else we set the offset
        offset = new Vector2((Time.time - GameManager_AlienHop.instance.timeDiff) * bgspeed, 0);
