...
#endif
        if (GameManager_AlienHop.instance == null)
            return;

        if (GameManager_AlienHop.instance.gameOver == true)
        {
            //we want only one ad to be shown so we put condition that when i is 0 we show ad.
            if (i == 0)
            {
                i++;
                GameManager_AlienHop.instance.gamesPlayed++;

                if (GameManager_AlienHop.instance.gamesPlayed >= vars.showInterstitialAfter)
                {
                    GameManager_AlienHop.instance.gamesPlayed = 0;
#if AdmobDef
