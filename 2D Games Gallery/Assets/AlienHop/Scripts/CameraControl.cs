...
public class CameraControl : MonoBehaviour {
    ...
    void Update()
    {
        if (GameManager_AlienHop.instance.gameOver == false)
        {
            if (!audioS.isPlaying)
            {
                audioS.Play();
        ...
        else if (GameManager_AlienHop.instance.gameOver == true)
        {
            if (audioS.isPlaying)
            {
                audioS.Stop();
    ...
    void FixedUpdate()
    {
        ...
        if (player == null || GameManager_AlienHop.instance.gameOver == true)
            return;
